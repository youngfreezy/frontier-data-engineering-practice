...
A run is identified by task, candidate, attempt, starting commit, requirements hash, dataset hash, and environment hash. Keep each candidate in its own clean worktree or container. Never compare candidates that started from different code, data, requirements, required gates, or required artifacts. When requirements change, start a new task ID and requirements hash, preserve the old ledger, and record a migration note.
...
1. Contract: requirements map to tests and evaluator assertions; unresolved and undisclosed semantics are recorded.
2. Evaluator sanity: reproduce the baseline; require the reference to pass and a no-op or semantic mutant to fail when those paths exist.
3. Identity and isolation: claim ownership; pin the start commit, requirements, dataset, environment, clean checkout, and required artifacts.
4. Grain and keys: assert output grain, business-key uniqueness, join cardinality, unmatched keys, null keys, and referential integrity.
5. Invariants: assert nulls, ranges, categories, volume, distribution, freshness, and aggregate checksums where the contract uses them.
6. Replay and recovery: rerun equivalent input, inject interruption and partial success, and test duplicates, backfills, and concurrent writers as applicable.
7. Schema: test declared types and nullability plus additive, missing, renamed, coerced, and incompatible fields.
8. Time: distinguish event, ingest, and processing time; pin time zone; test daylight-saving, skew, early, late, and out-of-order data.
9. Determinism: remove or pin wall-clock time, random IDs, unstable iteration, floating-point order, and unspecified tie-breaks.
10. Semantic equality: compare primary-key sets, typed invariants, and aggregate checksums before canonical sorting, null encoding, decimal scale, and hashing.
11. Plan, scale, layout, and cost: record rows, partitions, bytes, elapsed time, memory, spill or shuffle, file count, cost, and a plan fragment that proves the claimed access path.
12. Classification: record `solved`, completed semantic failure, timeout, OOM, infrastructure, malformed or missing artifact, or transport failure from raw evidence.
13. Snapshot: hash every decision-relevant artifact and revalidate it inside the approval and submission transitions.

Use the expanded catalog in [references/gates.md](references/gates.md) and reject the shortcuts in [references/anti-patterns.md](references/anti-patterns.md).

## Workflow

Steps 1–4, 6–8, and 10 are blocking. Step 5 selects task-specific gates. Step 9 is comparative and cannot rescue a blocked run.
...
9. Give an independent reviewer the task contract, final diff, test logs, output diff, execution plans, and outcome classification. The reviewer checks contract traceability, evidence, artifact integrity, plans, and classification; uncited concerns remain unverified leads and cannot introduce hidden criteria.
